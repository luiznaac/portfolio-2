...
export function PositionsTable({ positions }: { positions: Position[] }) {
  if (positions.length === 0) return <p className="text-sm text-slate-500">Nada consolidado.</p>;
        ...
        <tbody className="divide-y divide-white/5">
          {rows.map((p) => (
            <tr key={p.date}>
              ...
              <td className="py-2 pr-4 text-right text-principal">{formatBRL(p.principal)}</td>
              <td className="py-2 pr-4 text-right text-yield">{formatBRL(p.yield)}</td>
              <td className="py-2 pr-4 text-right text-tax">−{formatBRL(p.taxes)}</td>
