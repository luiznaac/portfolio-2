...
export function Dashboard() {
  ...
  if (bonds.error) return <p className="text-tax">Falha ao carregar: {String(bonds.error)}</p>;
  ...
  return (
    <div className="space-y-6">
      ...
      {schedule.isError && <p className="text-sm text-tax">{String(schedule.error)}</p>}
      {schedule.isSuccess && (
        ...
        return (
          <li key={it.to}>
            ...
            >
              ...
              <span className="min-w-0 flex-1 truncate text-sm text-slate-300">{it.name}</span>
