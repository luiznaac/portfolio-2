...
Repo-root `contracts/` holds JSON fixtures that are the cross-cutting source of truth for the
backend tests. `contracts/error-response.json` is the canonical domain-exception body: the
backend's `DomainExceptionContractTest` asserts the API serialises exactly it, so a shape change
edits the fixture once.
