...
class IndexService(
    ...
    private val indexGateway: IIndexGateway,
) {

    suspend fun fetchAllIndexes() = indexRepository.fetchAllIndexes()

    suspend fun fetchAllIndexValuesBy(indexId: IndexId) = indexRepository.fetchAllIndexValuesBy(indexId)

    suspend fun hydrateIndexValues(indexId: IndexId) =
        buildDates()
            .asFlow()
            .onEach { log.info("Fetching values for {} from {} to {}", indexId, it.first, it.second) }
            .map { indexGateway.getIndexValuesForDateRange(indexId, it.first, it.second) }
            .map { it.map { idx -> idx.toIndexValue() } }
            .onEach { indexRepository.persistIndexValues(indexId, it) }
